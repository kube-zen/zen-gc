	...
	lec := leaderelection.LeaderElectionConfig{
		...
		Callbacks: leaderelection.LeaderCallbacks{
			OnStartedLeading: func(ctx context.Context) {
				...
				klog.Infof("Became leader (identity: %s, namespace: %s, name: %s)",
					le.identity, le.namespace, le.name)
			...
			OnStoppedLeading: func() {
				...
				klog.Infof("Lost leadership (identity: %s)", le.identity)
			...
			OnNewLeader: func(identity string) {
				klog.Infof("New leader elected: %s (current identity: %s)", identity, le.identity)
			},
		},
	}
